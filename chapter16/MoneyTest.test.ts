...
describe('', () => {
  ...
  it('SumPlusMoney', () => {
    ...
    const bank = new Bank();
    bank.addRate('CHF', 'USD', 2);
    const sum: Expression = new Sum(fiveDollar, tenFranc).plus(fiveDollar);
  ...
  it('SumTimes', () => {
    ...
    const bank = new Bank();
    bank.addRate('CHF', 'USD', 2);
    const sum: Expression = new Sum(fiveDollar, tenFranc).times(2);
    const result: Money = bank.reduce(sum, 'USD');
    expect(result).toStrictEqual(Money.dollar(20));
  })

  it('PlusSameCurrencyReturnMoney', () => {
    const sum: Expression = Money.dollar(1).plus(Money.dollar(1));
    expect(sum.constructor.name).toBe('Money')
  })
});