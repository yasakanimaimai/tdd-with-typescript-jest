...
- ついに最初のTODOであった「5ドル + 10フラン = 10フラン」に着手した
  - まずはテストを書き、それに合わせるようにコードを変更
  - 次に一般化(より抽象度の高い型で宣言する)作業を末端から開始して頂点(テストケース)まで到達させた。

<br>

## chapter16 (将来の読み手を考えたテスト)
- SumTimesテストではMoneyから間接的にsumを作る方法もあったが、
読み手のことを考えて直接Sumからインスタンスを作成した。
- 最後に「5ドル + 5ドル がMoneyを返す」のテストを書いたが、
それを実現させるための綺麗な実装が思い浮かばず、またやらなくても進められるため除外した。
